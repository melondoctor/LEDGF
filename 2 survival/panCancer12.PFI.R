...
setwd("C:\\Users\\melon doctor\\Desktop\\98panCancer\\12.PFI")             #设置工作目录
...
for(i in levels(rt[,"CancerType"])){
  ...
  if(pValue<pFilter){
    ...
    #绘制生存曲线
    rt1 <- cbind(rownames(rt1),rt1)
    colnames(rt1)[1] <- "sample"
    write.table(rt1,file = paste0("PFI.",i,".txt"),
    ...
    pdf(file=paste0("PFI.",i,".pdf"),onefile = FALSE, width = 6, height =5)
    print(surPlot)
    dev.off()
  }
  #cox分析
